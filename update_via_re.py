...
with open("src/components/dashboard/AttendeeDashboard.tsx", "r", encoding="utf-8") as f:
    text = f.read()

# Replace the space-y-4 div with grid
m1 = re.search(r'\{\/\*\s*SECTION:\s*LIVE MAP.*?\*\/.*?<div className="space-y-4">.*?<Card className="bg-slate-900 border-white/5[^>]*>', text, re.DOTALL)
if m1:
    old_top = m1.group(0)
    new_top = old_top.replace('space-y-4', 'space-y-6')
    new_top = new_top.replace('                   <Card', '               <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6">\n                  <div className="flex flex-col">\n                   <Card')
    new_top = new_top.replace('group">', 'group h-full flex flex-col min-h-[400px]">')
    text = text.replace(old_top, new_top)
    print("M1 replaced")
else:
    print("M1 failed")

# Replace the closing tag and add the intelligence section

m2 = re.search(r'(\s*Reset View\s*</Button>\s*)\}\)\s*</div>\s*</div>\s*</Card>\s*</div>', text, re.DOTALL)
...
if m2:
    ...
    old_end = m2.group(0)
    new_end = m2.group(1) + intel_col
    text = text.replace(old_end, new_end)
    print("M2 replaced")
else:
    print("M2 failed")
    
with open("src/components/dashboard/AttendeeDashboard.tsx", "w", encoding="utf-8") as f:
    f.write(text)
